...
export default async function Status({ className }: StatusProps) {
	const projects = await getStatus();
	...
	const status =
		ichess?.incidents && ichess.incidents.length > 0
	...
	return (
		<a
			...
			className={cn(
				"flex flex-row items-center justify-center gap-x-3 px-3 py-2 transition w-full sm:w-fit outline outline-[0.75px] outline-transparent hover:bg-gray-300 rounded-md font-medium",
				className,
			)}
