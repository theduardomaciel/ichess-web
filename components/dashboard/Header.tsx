...
export default function DashboardHeader() {
	...
	return (
		<header
			className={cn(
				"flex flex-col md:flex-row md:justify-between md:items-center p-wrapper md:h-36 md:py-0 gap-[var(--wrapper)] items-start max-h-[calc(5.6rem+var(--wrapper)/2)] transition-all duration-300 overflow-y-hidden",
		...
		>
			<div className="flex flex-row items-center justify-between w-full md:w-[20%] h-14">
				<Link href={`/dashboard`}>
					<Logo />
				</Link>
				<button
			...
			<ul
				className={cn(
					"flex flex-row items-center justify-between md:justify-center md:gap-12 w-full transition-[opacity,transform]",
					{
			...
			>
				{SECTIONS.map(({ title, href }) => (
					<li
						key={title}
						className={cn(
							"text-base font-semibold tracking-tight text-center",
							{
			...
			<Link className="w-full md:w-[20%]" href={`/dashboard/add`}>
				<Button
					variant={"outline"}
					className={cn(
						"text-xs uppercase border-primary-100 w-full transition-[opacity,transform,background-color] duration-300",
						{
							"opacity-0 translate-x-[-1rem]": !isExpanded,
							"opacity-100 translate-y-0": isExpanded,
