...
function FormSection({ children, ...rest }: FormSectionProps) {
	return (
		<div className="flex flex-col lg:flex-row items-start justify-start gap-9 lg:gap-16 w-full">
			<FormProgress {...rest} />
...
function FormProgress({ title, fields }: FormSectionProps) {
	return (
		<div className="flex flex-col w-full lg:sticky top-4 left-0 lg:w-2/5 bg-background-600 rounded-2xl border border-primary-100">
			<div className="flex flex-row items-center justify-start px-6 py-[18px] bg-primary-100 rounded-tl-2xl rounded-tr-2xl">
				<h6 className="font-extrabold text-base lg:text-lg">{title}</h6>
			</div>
